...
type VueAffichage = "liste" | "grille";
...
function formatTaille(taille: number | null): string {
  if (!taille || taille <= 0) {
    return "—";
  }
...
function iconeDocument(
  ...
  const extension = extensionNormalisee(document);
...
export default function DocumentsPage() {
  ...
  const [folders, setFolders] =
    useState<DocumentFolderNode[]>([]);

  const [categories, setCategories] =
    useState<string[]>([]);

  const [stats, setStats] =
    useState<DocumentStats>(STATS_VIDES);
  ...
  const [dossiersOuverts, setDossiersOuverts] =
    ...
    async (silent = false) => {
      ...
      try {
        ...
        const [
          ...
          getDocumentFolders(scope),
          getDocumentCategories(scope),
          getDocumentStats(scope),
        ]);
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <nav className="space-y-1">
              <SidebarButton
                ...
                count={stats.total}
                onClick={afficherTous}
              />

              <SidebarButton
                active={
                  filters.favoris === true
                }
                icon={
                  ...
                  {document.archive
                    ? canEdit && (
                        <button
                          ...
                          className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 font-semibold text-white disabled:opacity-50"
                        >
