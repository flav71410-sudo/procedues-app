...
function obtenirNomPrestataire(
...
export default function NouvelleMaintenancePage() {
  ...
  async function rechargerOptions(
    ...
    try {
      ...
      const donnees =
        await getMaintenanceFormOptions({
          magasinId:
            magasinActif.id,
          tousMagasins:
            false,
      ...
      /*
       * Les prestataires sont chargés séparément
       * et directement depuis Supabase.
       */
      ...
      setFormulaire(
        (actuel) => {
          if (
            !actuel.prestataire_id
          ) {
            return actuel;
          }

          const existe =
            donnees.prestataires.some(
              (item) =>
                item.id ===
                actuel.prestataire_id
            );
  ...
  async function rechargerPrestataires() {
    if (
      authLoading ||
      vueTousMagasins ||
      !magasinActif?.id
    ...
    try {
       ...
       * On ne passe plus par maintenanceService
       * pour la liste des prestataires.
       */
      const {
        data,
        error,
      ...
      console.error(
        "Erreur chargement direct prestataires Supabase :",
        error
      );
  ...
  useEffect(() => {
    ...
    /*
     * Rafraîchissement automatique fiable :
     * toutes les 5 secondes, uniquement la liste
     * des prestataires est rechargée.
     */
    const intervalId =
      window.setInterval(
        () => {
          void rechargerPrestataires();
        },
        3000
      );
